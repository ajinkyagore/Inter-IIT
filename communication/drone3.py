...
def update_firebase(str_gps,count):
    ...
    firebase.put('/drone3/obj'+str(count),"lat",lat)
    firebase.put('/drone3/obj'+str(count),"lng",lon)
    
    
    firebase.put('/',"count3",count)
...
while True:
    if obj_detected:
        ...
        sleep(1)
    count_total=firebase.get('/count1',None)+firebase.get('/count2',None)+count
    firebase.put('/',"countT3",count_total)
